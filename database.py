...
def _init_cloud_db():
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS feedbacks (id INT AUTO_INCREMENT PRIMARY KEY, from_id VARCHAR(100) NOT NULL, from_role VARCHAR(20) NOT NULL, to_id VARCHAR(100) NOT NULL, doctor_id VARCHAR(100) NOT NULL DEFAULT '', content TEXT NOT NULL, is_read INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, INDEX idx_to_id (to_id), INDEX idx_to_doctor (to_id, doctor_id), CONSTRAINT fk_feedbacks_from FOREIGN KEY (from_id) REFERENCES users(user_id) ON DELETE CASCADE, CONSTRAINT fk_feedbacks_to FOREIGN KEY (to_id) REFERENCES users(user_id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4")
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS feedback_read_progress (id INT AUTO_INCREMENT PRIMARY KEY, viewer_id VARCHAR(100) NOT NULL, patient_id VARCHAR(100) NOT NULL, doctor_id VARCHAR(100) NOT NULL DEFAULT '', last_read_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY unique_viewer_patient_doctor (viewer_id, patient_id, doctor_id), CONSTRAINT fk_read_progress_viewer FOREIGN KEY (viewer_id) REFERENCES users(user_id) ON DELETE CASCADE, CONSTRAINT fk_read_progress_patient FOREIGN KEY (patient_id) REFERENCES users(user_id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4")
...
def _init_sqlite_db():
    ...
    with conn:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS feedbacks (id INTEGER PRIMARY KEY AUTOINCREMENT, from_id TEXT NOT NULL, from_role TEXT NOT NULL, to_id TEXT NOT NULL, doctor_id TEXT NOT NULL DEFAULT '', content TEXT NOT NULL, is_read INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (from_id) REFERENCES users(user_id) ON DELETE CASCADE, FOREIGN KEY (to_id) REFERENCES users(user_id) ON DELETE CASCADE)")
        # ★ 新增：跟 MySQL 那边一样，各端独立记录已读进度
        conn.execute("CREATE TABLE IF NOT EXISTS feedback_read_progress (id INTEGER PRIMARY KEY AUTOINCREMENT, viewer_id TEXT NOT NULL, patient_id TEXT NOT NULL, doctor_id TEXT NOT NULL DEFAULT '', last_read_at TEXT DEFAULT (datetime('now')), UNIQUE(viewer_id, patient_id, doctor_id), FOREIGN KEY (viewer_id) REFERENCES users(user_id) ON DELETE CASCADE, FOREIGN KEY (patient_id) REFERENCES users(user_id) ON DELETE CASCADE)")
